...
export default function Page() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // email and password are already coming from the React state
            // which is populated by the user's input in the form fields
            const response = await fetch(`${domain}/login`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ email, password }), // Using values from state that were input by user
            });
            ...
            if (response.ok) {
                // Login successful - store any tokens or user info if needed
